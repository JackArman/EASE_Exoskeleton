...
void loop() {
  ...
  while (dial != DIAL_OFF ) {
    ...
    float deadband = 0.1;
    if (fabs(R_hip[RgaitIndex] * 1.3) <= deadband) {
      if (exitCond) {
        gaitTime = 0;
        break;
      }
  ...
  for (int i = 1; i < 20; i++) {
    ...
    double R_hip_ang = -(R_hip[LgaitIndex] + 1.3f) * (1 - i/20.0f);
    double R_knee_ang = -(R_knee[leftKnee] * 0.9f) * (1 - i/20.0f);
    double L_hip_ang = (R_hip[RgaitIndex] + 1.3f) * (1 - i/20.0f);
    double L_knee_ang = (R_knee[rightKnee] * 0.9f) * (1 - i/20.0f);

    sendMITCommand(R_hip_ang,  v_des, kp, kd, -torque_ff_L_HIP * (1 - i/20.0f), MOTOR_ID_LEFT_HIP);
    delayMicroseconds(150); drainRXUntil(400);

    sendMITCommand(R_knee_ang,  v_des, kp, kd, -torque_ff_L_KNEE * (1 - i/20.0f), MOTOR_ID_LEFT_KNEE);
    delayMicroseconds(150); drainRXUntil(400);

    sendMITCommand(L_hip_ang,  v_des, kp, kd, torque_ff_R_HIP * (1 - i/20.0f), MOTOR_ID_RIGHT_HIP);
    delayMicroseconds(150); drainRXUntil(400);

    sendMITCommand(L_knee_ang,  v_des, kp, kd, torque_ff_R_KNEE * (1 - i/20.0f), MOTOR_ID_RIGHT_KNEE);
    delayMicroseconds(200); drainRXUntil(500);
    delay(200);
  }
