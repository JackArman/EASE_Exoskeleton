...
void loop() {
  ...
  for (int i = 1; i < 20; i++) {
  //   sendMITCommand(-(R_hip[GAIT_LENGTH - LgaitIndex] + 0.02) * i/20,  v_des, kp, kd, torque_ff, MOTOR_ID_LEFT_HIP);
  //   sendMITCommand(-(R_knee[GAIT_LENGTH - LgaitIndex] * .8) * i/20, v_des, kp, kd, torque_ff, MOTOR_ID_LEFT_KNEE);
  //   sendMITCommand((R_hip[GAIT_LENGTH - RgaitIndex] + 0.02) * i/20, v_des, kp, kd, torque_ff, MOTOR_ID_RIGHT_HIP);
  //   sendMITCommand((R_knee[GAIT_LENGTH - RgaitIndex] * .8) * i/20,v_des, kp, kd, torque_ff, MOTOR_ID_RIGHT_KNEE);
  ...
  while (true) {
    // sendMITCommand(-(R_hip[GAIT_LENGTH - LgaitIndex] + 0.02),  v_des, kp, kd, torque_ff, MOTOR_ID_LEFT_HIP);
    // sendMITCommand(-(R_knee[GAIT_LENGTH - LgaitIndex] * .8), v_des, kp, kd, torque_ff, MOTOR_ID_LEFT_KNEE);
    // sendMITCommand((R_hip[GAIT_LENGTH - RgaitIndex] + 0.02), v_des, kp, kd, torque_ff, MOTOR_ID_RIGHT_HIP);
    // sendMITCommand((R_knee[GAIT_LENGTH - RgaitIndex] * .8),v_des, kp, kd, torque_ff, MOTOR_ID_RIGHT_KNEE);
    int leftKnee = (LgaitIndex + offset) % GAIT_LENGTH;
    int rightKnee = (RgaitIndex + offset) % GAIT_LENGTH;
